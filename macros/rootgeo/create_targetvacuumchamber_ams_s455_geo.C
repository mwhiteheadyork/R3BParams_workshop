...
void create_targetvacuumchamber_ams_geo(const char *geoTag = "s455") {
  ...
  TString geoFileName = "../../target/";
  geoFileName = geoFileName + geoTag + "vacuumchamber.geo.root";
  // --------------------------------------------------------------------------
  ...
  for (Int_t n = 0; n < nbdet; n++) {
    ...
    AmsDet[n] =
        gGeoManager->MakeBox(buffer, pMedVac, 7.05 / 2., 4. / 2., 0.0302 / 2.);
  }
  ...
  Shielding =
      gGeoManager->MakeBox(buffer, pMedGold, 7.4 / 2.0, 4.1 / 2., 0.0020 / 2.);
