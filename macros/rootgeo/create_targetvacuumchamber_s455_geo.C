...
void create_targetvacuumchamber_geo(const char *geoTag = "s455") {
  ...
  TString geoFileName = "../../target/targetvacuumchamber_";
  ...
  TGeoShape *pVCCone =
      new TGeoCone("Cone", (33.688 + 4.) / 2., 0., 4.6, 0., 7.8);

  TGeoCombiTrans *t_cone = new TGeoCombiTrans(
      "t_cone", 0., 0., 442. / 2.0 + (33.688 + 4.) / 2., fRefRot);
  t_cone->RegisterYourself();

  TGeoCompositeShape *pVCWorld =
      new TGeoCompositeShape("VCbox", "CenterPart:t_tube + Cone:t_cone");
  ...
  Double_t offsetZ =
      -0.50; // offset with respect to LH2 target cell, entrance at (0,0,0)
  ...
  Double_t LH2targetR =
      1.5 - thinMylar2 * 2.0; // Radius of 1.5 cm - mylar cylindre
  ...
  TGeoVolume *pChamberEndPlateLog =
      new TGeoVolume("ChamberEndPlateLog", pChamberEndPlate, pMed3);
  ...
  TGeoVolume *pTargetEndPlateInLog =
      new TGeoVolume("TargetEndPlateLog", pTargetEndPlateIn, pMed1);
  ...
  TGeoSphere *SphereChamber =
      new TGeoSphere("SphereChamber", 26.1, 26.4, 15.26, 90., 0., 360.);
  // TGeoVolume* pSphereChamberLog = new
  // TGeoVolume("pSphereChamberLog",SphereChamber, pMed2);
  ...
  /*  TGeoVolume* pChamberConeLog = new
    TGeoVolume("ChamberConeLog",pChamberCone, pMed2);
