...
void create_fi5_geo(const char *geoTag) {
  ...
  TString geoFileName = "../../fibers/fi5_";
  ...
  TGeoShape *pGFITube = new TGeoBBox("GFITube", fiber_thickness / 2,
                                     detector_height / 2, fiber_thickness / 2);

  TGeoVolume *pGFILog = new TGeoVolume("FI5Log", pGFITube, pMed35);

  TGeoShape *pGFITubeActive =
      new TGeoBBox("GFITubeActive", (fiber_thickness * .98) / 2,
                   detector_height / 2 - 0.0001, (fiber_thickness * .98) / 2);
  ...
  for (int fiber_id = 0; fiber_id < detector_width / fiber_thickness;
    ...
    pGFILogWorld->AddNode(
        pGFILog, fiber_id,
        new TGeoCombiTrans(
            "", -detector_width / 2 + (fiber_id + .5) * fiber_thickness, 0, 0,
            new TGeoRotation()));
  }
