...
void create_target_geo(const char *geoTag = "SofH2") {
  ...
  TString geoFileName = "../../target/target_";
  ...
  if (0 == strncmp(geoTag, "LeadTarget", 10))
    ConstructGeometry1(lead); // for s318
  // if (geoTag == "Para")       ConstructGeometry2(pMed2,pMed2); // equivalent
  // to no target
  if (0 == strncmp(geoTag, "Para", 4))
    ConstructGeometry2(pMed38, pMed2);
  if (0 == strncmp(geoTag, "Para45", 6))
    ConstructGeometry3(pMed38, pMed2);
  if (0 == strncmp(geoTag, "LiH", 3))
    ConstructGeometry4(pMed2, pMed15, pMed3, pMedAu);
  if (0 == strncmp(geoTag, "CTarget", 7))
    ConstructGeometry5(pMedCarbon); // for s318
  if (0 == strncmp(geoTag, "CH2Target", 9))
    ConstructGeometry6(pMed38); // for s318
  if (0 == strncmp(geoTag, "ETTarget", 6))
    ConstructGeometry7(pMed); // for s318, empty target
  // p2p sofia
  if (0 == strncmp(geoTag, "SofH2", 5))
    ConstructGeometrySOFIAp2p(pMed, pMed3);
...
void ConstructGeometry1(TGeoMedium *lead) {
  ...
  TGeoVolume *pTargetEnveloppe_log = new
  TGeoVolume("TargetEnveloppe",pTargetEnveloppe, pMed2);
...
void ConstructGeometry2(TGeoMedium *pMed38, TGeoMedium *pMed2) {
  ...
  TGeoVolume *pTargetEnveloppe_log =
      new TGeoVolume("TargetEnveloppe", pTargetEnveloppe, pMed2);
  pTargetEnveloppe_log->SetVisLeaves(kTRUE);
  ...
  phi1 = 0.000000;
  phi2 = 360.000000;
  TGeoShape *pParafin0deg =
      new TGeoTubeSeg("Parafin0deg", rmin, rmax, dz, phi1, phi2);
...
void ConstructGeometry3(TGeoMedium *pMed38, TGeoMedium *pMed2) {
  ...
  theta = 45.000000;
  phi = -180.000000;
  TGeoShape *pTargetEnveloppe =
      new TGeoPara("TargetEnveloppe", dx, dy, dz, alpha, theta, phi);
  TGeoVolume *pTargetEnveloppe_log =
      new TGeoVolume("TargetEnveloppe", pTargetEnveloppe, pMed2);
  pTargetEnveloppe_log->SetVisLeaves(kTRUE);
  TGeoCombiTrans *pGlobal = GetGlobalPosition(pMatrix2);
  ...
  theta = 45.000000;
  phi = -180.000000;
  TGeoShape *pPara45deg =
      new TGeoPara("Para45deg", dx, dy, dz, alpha, theta, phi);
...
void ConstructGeometry4(TGeoMedium *pMed2, TGeoMedium *pMed15,
  ...
  dz = 3.0; // this means the origine (0,0,0) starts at the edge of the Mylar
            // entrance foil and the H2 liq.
  // Rotation:
  thx = 90.000000;
  phx = 0.000000;
  thy = 90.000000;
  ...
  TGeoVolume *pTarget1_log =
      new TGeoVolume("Target1", pTarget1, pMed15); // Mylar
  ...
  TGeoVolume *pTarget3_log =
      new TGeoVolume("Target3", pTarget3, pMed15); // Mylar
  ...
  TGeoVolume *pTarget4_log =
      new TGeoVolume("Target4", pTarget4, pMedAu); // Gold
...
void ConstructGeometry5(TGeoMedium *pMedCarbon) {
  ...
  TGeoVolume *pCarbonTarget_log =
      new TGeoVolume("CarbonTarget_log", pCarbonTarget, pMedCarbon);
...
void ConstructGeometrySOFIAp2p(TGeoMedium *pMed1, TGeoMedium *pMed2) {
  ...
  cout << endl;
  cout << "-I- R3BTarget:: ConstructGeometry() " << endl;
  cout << "-I- R3BTarget Target type: H2 for p2p with SOFIA, length: " << Length
       << endl;
  cout << endl;
  ...
  TGeoShape *pTarget1 = new TGeoTube("Target1", rmin, rmax, dz);
  TGeoVolume *pTarget1_log =
      new TGeoVolume("Target1", pTarget1, pMed2); // Mylar
  pTarget1_log->SetVisLeaves(kTRUE);
  // pTargetEnveloppe_log->AddNode(pTarget1_log, 0, pMatrix4);
  ...
  TGeoShape *pTarget3 = new TGeoTube("Target3", rmin, rmax, dz);
  TGeoVolume *pTarget3_log =
      new TGeoVolume("Target3", pTarget3, pMed2); // Mylar
  pTarget3_log->SetVisLeaves(kTRUE);
  // pTargetEnveloppe_log->AddNode(pTarget3_log, 0, pMatrix8);
  // pTargetEnveloppe_log->AddNode(pTarget3_log, 1, pMatrix10);
  ...
  phi2 = 360.000000;
  TGeoShape *pTarget4 = new TGeoCone("Target4",dz,rmin,rmax,rmin2,rmax2);
  TGeoVolume *pTarget4_log = new TGeoVolume("Target4",pTarget4, pMed2);    //
Gold
 // pTarget4_log->SetVisLeaves(kTRUE);
//gTop->AddNode(pTarget4_log, 0, pMatrix12);
//pWorld->AddNode(pTarget4_log, 0, pMatrix12);
  ...
  TGeoVolume *pTargetEnveloppe_log = new TGeoVolume("TargetEnveloppe",pTargetH2,
pMed2); // H2 pTargetEnveloppe_log->SetVisLeaves(kTRUE);
