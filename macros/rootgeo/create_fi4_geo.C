...
void create_fi4_geo(const char *geoTag) {
  ...
  TString geoFileName = "../../fibers/fi4_";
  ...
  TGeoShape *pGFITube =
      new TGeoTube("GFITube", 0, fiber_thickness / 2, detector_height);

  TGeoVolume *pGFILog = new TGeoVolume("FI4Log", pGFITube, pMed35);

  TGeoShape *pGFITubeActive =
      new TGeoTube("GFITubeActive", 0, (fiber_thickness * .97) / 2,
                   detector_height - 0.0001);
  ...
  for (int fiber_id = 0; fiber_id < detector_width / fiber_thickness;
       fiber_id++) {
    pGFILogWorld->AddNode(
        pGFILog, real_fiber_id++,
        new TGeoCombiTrans(
            "", -detector_width / 2 + (fiber_id + .5) * fiber_thickness, 0,
    ...
    pGFILogWorld->AddNode(
        pGFILog, real_fiber_id++,
        new TGeoCombiTrans(
            "", -detector_width / 2 + (fiber_id + 1) * fiber_thickness, 0,
            (fiber_thickness * TMath::Cos(30 / 180 * TMath::Pi())) / 2,
            pMatrixTube));
  }
  ...
  /*pGFILogWorld->AddNode(pGFILog, detector_width / fiber_thickness - 1,
                new TGeoCombiTrans("",
                        -detector_width / 2 + (detector_width / fiber_thickness
     - 1 + .5) * fiber_thickness, 0,
                        -(fiber_thickness * TMath::Cos(30/180*TMath::Pi())) / 2,
                        pMatrixTube
                )
                );*/
