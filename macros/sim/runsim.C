...
void runsim(Int_t nEvents = 0) {
  ...
  Bool_t fSofiaDigitizer = false; // Apply hit digitizer task
  ...
  TString fTrackerGeo = "ams_target_s455.geo.root";
  ...
  Bool_t fMwpc2 = false; // MWPC2 Detector
  ...
  Bool_t fMwpc3 = false; // MWPC3 Detector
  TString fMwpc3Geo = "mwpc_3.geo.root";

  Bool_t fSofTofWall = false; // Sofia ToF-Wall
  TString fSofTofWallGeo = "sof_tof_v19.geo.root";

  Bool_t fNeuLand = false; // NeuLand Detector
  TString fNeuLandGeo = "neuland_s2018.geo.root";

  // ---- End of Configuration area   ---------------------------------------

  // ---- Stable part   -----------------------------------------------------
  TString dir = gSystem->Getenv("VMCWORKDIR");
  TString pardir = dir + "/../R3BParams";
  pardir.ReplaceAll("//", "/");

  TString r3b_geomdir = pardir + "/geometry/";
  ...
  parIo1->open((r3b_geomdir + fSetupFile).Data(), "in");
  ...
  if (fMwpc0) {
    if (mwpc0Par) {
      ...
      run->AddModule(
          new R3BSofMwpc0(fMwpc0Geo, {mwpc0Par->GetPosX(), mwpc0Par->GetPosY(),
  ...
  if (fMwpc1) {
    if (mwpc1Par) {
      ...
      run->AddModule(
          new R3BSofMwpc1(fMwpc1Geo, {mwpc1Par->GetPosX(), mwpc1Par->GetPosY(),
  ...
  if (fTwim) {
    if (twimPar) {
      ...
      run->AddModule(
          new R3BSofTwim(fTwimGeo, {twimPar->GetPosX(), twimPar->GetPosY(),
                                    twimPar->GetPosZ(), rtwim}));
    } else
  ...
  if (fMwpc2) {
    if (mwpc2Par) {
      ...
      run->AddModule(
          new R3BSofMwpc2(fMwpc2Geo, {mwpc2Par->GetPosX(), mwpc2Par->GetPosY(),
                                      mwpc2Par->GetPosZ(), rmwpc2}));
    } else
  ...
  if (fMwpc3) {
    ...
    if (mwpc3Par) {
      ...
      run->AddModule(
          new R3BSofMwpc3(fMwpc3Geo, {mwpc3Par->GetPosX(), mwpc3Par->GetPosY(),
                                      mwpc3Par->GetPosZ(), rmwpc3}));
    } else {
      rmwpc3->RotateY(-18.);
  ...
  if (fSofTofWall) {
    ...
    if (tofwPar) {
      ...
      run->AddModule(new R3BSofTofW(
          fSofTofWallGeo,
          {tofwPar->GetPosX(), tofwPar->GetPosY(), tofwPar->GetPosZ(), rtof}));
    } else {
      rtof->RotateY(-18.);
  ...
  if (fGenerator.CompareTo("box") == 0) {
    ...
    FairIonGenerator *ionGen =
        new FairIonGenerator(82, 208, 82, 1, 0., 0., 1.09, 0., 0., -75.);
    primGen->AddGenerator(ionGen);
  }

  if (fGenerator.CompareTo("ascii") == 0) {
    R3BAsciiGenerator *gen =
        new R3BAsciiGenerator((pardir + "/input/" + fEventFile).Data());
  ...
  if (fGenerator.CompareTo("inclroot") == 0) {
    R3BINCLRootGenerator *gen =
        new R3BINCLRootGenerator((pardir + "/input/" + fEventFile).Data());
